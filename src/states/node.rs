...
impl Node {
    ...
    fn handle_node_identify(&mut self,
        ...
        if let Some(prefix) = self.routing_table().find_section_prefix(public_id.name()) {
    ...
    fn handle_expect_candidate(&mut self,
        ...
        if self.routing_table().should_join_our_section(candidate_id.name()).is_err() {
            let relocated_name =
                self.routing_table().assign_to_min_len_prefix(candidate_id.name());
    ...
    fn handle_section_update(&mut self,
        ...
        let members =
            if let Some(section) = self.routing_table().section_with_prefix(&prefix) {
    ...
    fn send_candidate_approval(&mut self) -> Result<(), RoutingError> {
        let (candidate_id, client_auth, sections) = match self.route_mgr.verified_candidate_info(&self.peer_mgr) {
            Err(_) => {
